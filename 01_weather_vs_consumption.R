...
ggplot(temp, aes(x = tavg, y = consumption))+
  ...
  labs(title = 'Погода та споживання електроенергії', 
       subtitle = str_wrap('Кожна точка на графіку — один день у 2016 році. Позиція точки визначається середньою температурою в цей день (вісь Х, градуси Цельсія) та обсягом споживання електроенергії (вісь Y, ГВт)', 110),
